...
| 43 | [Apparel Group AI/ML Mock Test (timed)](43_apparel_group_mock_test.md) | 6 | **NEW.** Companion to Ch.42 — a fresh 25-question, ~40-min timed dry-run in Mettl/iMocha style with a separate answer key + explanations; every key adversarially re-solved. Take it under exam conditions, then score against the key. |
| 44 | [Cognine — Senior ML Engineer (AWS) first-round technical](44_cognine_senior_mle_aws.md) | ~16 | **NEW.** Company intel (Cognine: AI services + GCC, Schaumburg/Hyderabad, logistics/healthcare/fintech/manufacturing) + a full AWS Senior-MLE technical prep: reference architecture (SageMaker/Lambda/S3/Athena/Glue/CloudWatch/Terraform/Airflow) you can whiteboard, serving-pattern decision table, worked fraud/credit-risk example, MLOps CI/CD loop, drift/monitoring, GenAI/RAG breadth, live-coding snippets, a freight-ETA system design, 30-Q rapid-fire bank + morning-of cheatsheet. |
| 45 | [Google — Staff AI/ML Engineer, YouTube Create](45_google_youtube_create_staff_aiml.md) | ~50 | **NEW.** L6 pack: JD decode, YouTube Create product/tech intel, the full interview loop, coding bank, CV/video ML, diffusion (Imagen/Veo), ASR/audio, on-device ML, 6 worked ML system designs, Googleyness & leadership, mock design transcript, 40-Q rapid-fire. |
| 46 | [Logic20/20 — Offshore Senior MLE (SDG&E Vegetation Management)](46_logic2020_senior_mle_sdge_veg.md) | ~28 | **NEW.** Full 2-round notebook for the SDG&E wildfire/veg-management MLE seat: Logic20/20 + SDG&E intel (VRI, TreeVision, WiNGS, WMP/OEIS/CPUC, HFTD, GO 95, grow-in vs fall-in, PSPS), the veg-ML domain (LiDAR point clouds, aerial/satellite segmentation, encroachment risk, geospatial stack, IoU/mAP/PR-AUC), resume→role skill-map + honest-gap bridges, a whiteboard-able veg-risk reference architecture, worked system design, live-coding, per-interviewer game plans (Linh Nguyen / Christian White), ~54-Q bank, market context, morning-of cheatsheet, and a do-NOT-state-as-fact honesty section. |

| 54 | [Smartsheet · Senior AI/ML Ops — Round 1: Python CoderPad](54_smartsheet_python_coderpad.md) | ~90 | Provenance-first pack for a 60-min CoderPad coding round: reported format, the LLM-assist quirk, Smartsheet's object model, 60+ Python-internals questions, 22 classics, MLOps utilities, STAR stories, hour-of cheatsheet. |
| 55 | [Smartsheet · Senior AI/ML Ops — Round 2: Design & Domain](55_smartsheet_r2_design_domain.md) | ~200 | **NEW.** The architecture-and-depth round. Round decode + 60-min playbook in 3 variants; company delta for a design round; one complete 40-min flagship platform design; 12 more worked designs (permission-aware RAG, text-to-formula, retraining automation, observability, LLM gateway, brownfield migration, agent runtime, knowledge graph, fine-tuning, CI/CD+IaC, multi-region, cost); four domain primers (Databricks/Unity Catalog/MLflow/Mosaic AI · Bedrock + AWS ML plane + Monte Carlo + drift statistics · Kubernetes/serving/promotion gates/Terraform/design-to-cost · RAG/vector stores/GraphRAG/agent ops/LLMOps governance); 205 probes by JD line with 10 debugging ladders and 5 SQL exercises; platform stories, the gap playbook and the consistency sheet; hour-of cheatsheet. |
| 56 | [Smartsheet Round 2 in Simple English (with diagrams)](56_smartsheet_r2_simple_english.md) | ~80 | **NEW.** The plain-English companion to Ch.55 — short sentences, jargon explained on first use, **61 Mermaid diagrams + 66 ASCII diagrams**. Every question in one drillable shape (question → what they are really checking → a short spoken answer → if they push deeper). Covers the round and the panel, one master platform diagram, 14 core ideas, the Databricks stack, AWS/Bedrock/monitoring, 60 likely questions, 5 full designs, and the stories/gaps/pre-flight checklist. |

**Total: ~924 pages** across 49 indexed chapters (47–53 are listed in the README).
...
- **Apparel Group (Dubai, AI ML Engineer — online assessment)** — Chapter [42](42_apparel_group_ml_assessment.md): a self-test bank of 163 verified questions for the sub-45-min Mettl/iMocha-style technical screen, plus platform/proctoring format intel and company intel. Drill the first six sections hardest (core ML, metrics, classical algos, Python/sklearn, stats, deep learning), then take the timed mock in [43](43_apparel_group_mock_test.md) as a dry-run.
- **Cognine (Hyderabad/US, Senior ML Engineer — AWS)** — Chapter [44](44_cognine_senior_mle_aws.md): company intel + first-round AWS technical prep. Pair with [11](11_aws_azure.md) (AWS depth), [10](10_mlops_llmops.md) (MLOps), [14](14_monitoring_drift.md) (drift), [16](16_system_design.md) (system design), [24](24_apache_airflow.md) (Airflow), [38](38_lambda_cold_start_ml.md) (Lambda inference). Whiteboard the §6 reference architecture and the §9 freight-ETA design until fluent.
- **Logic20/20 (offshore, Senior MLE — SDG&E vegetation/wildfire)** — Chapter [46](46_logic2020_senior_mle_sdge_veg.md): full 2-round notebook (Linh Nguyen technical + Christian White technical/fit). Pair with [11](11_aws_azure.md) (AWS + Azure), [10](10_mlops_llmops.md) (MLOps), [14](14_monitoring_drift.md) (drift — vegetation drifts by construction), [16](16_system_design.md) (system design), [29](29_ensemble_models.md) (XGBoost/LightGBM), [44](44_cognine_senior_mle_aws.md) (sibling consulting-MLE chapter). Whiteboard the §6 veg-risk reference architecture and rehearse the §5.2 honest-gap sentence; re-read §15 (do-NOT-state-as-fact) before both calls.
- **Smartsheet (Bangalore GCC, Senior AI/ML Ops Engineer)** — a full two-round pack: [54](54_smartsheet_python_coderpad.md) for the Python CoderPad round, [55](55_smartsheet_r2_design_domain.md) for the AI/ML Ops **Design & Domain** round, and [56](56_smartsheet_r2_simple_english.md) for the same round in plain English with diagrams. Pair with [10](10_mlops_llmops.md) (MLOps), [14](14_monitoring_drift.md) (drift), [11](11_aws_azure.md) (AWS/Azure), [12](12_kubernetes_ray.md) (K8s), [07](07_rag.md)/[27](27_rag_evaluation.md) (RAG + eval), [41](41_llm_gateway.md) (LLM gateway), [16](16_system_design.md) (system design). Read 56 first, 55 for depth.
